...
const discountHandler = require('../DiscountHandler/discount-handler');
...
const editFieldsFunction = async (OrderUrl, dataArr, orderUrlArr, dataUserPhoneArr) => {
    try {
        ...
        const phoneIndex = dataUserPhoneArr.findIndex((el) => el.phone === dataUserNamePhone.phone);
        ...
        const newData = dataArr.filter((el) => el.phone === dataUserNamePhone.phone)[0];
        ...
        if (newData) {
            ...
            const dataSetFields = await postPutData({
                headers, url: OrderUrl, method: 'put', body: bodyData,
            });
